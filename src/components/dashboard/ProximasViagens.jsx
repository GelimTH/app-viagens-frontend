...
export default function ProximasViagens({ viagens }) {
  const proximasViagens = viagens
    // MUDANÇA AQUI: Adicionamos `&& v.status !== 'reprovado'` para excluir as viagens reprovadas
    .filter(v => (new Date(v.dataIda) >= new Date() || v.status === 'em_analise') && v.status !== 'reprovado')
    .slice(0, 5);
            ...
            {proximasViagens.map((viagem) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <Badge variant={statusConfig[viagem.status].variant}>
